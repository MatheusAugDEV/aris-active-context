...
- F32.Z6 future MCP read-only configuration final pre-apply dry-run simulation is warn-passed; the simulation remains artifact-only and simulation-only, with planned config content, planned hash, rollback handle, ledger entry, and denylist materialized, and F32.Z7 — Future MCP Read-Only Configuration Final Pre-Apply Dry-Run Simulation Review Gate is next.
