...
- reason: `runtime isolation is review-only and rollback planning is now the first remaining gate`

- future_reentry_option: `Proceed to rollback plan review only after the runtime contract remains review-only`
...
- future_reentry_option: `Proceed to read-only enforcement review only after the sandbox contract remains review-only`
...
- future_reentry_option: `Proceed to source-safety closure before any approval discussion`
...
- future_reentry_option: `If the local snapshot changes, re-intake it before the safety audit`
