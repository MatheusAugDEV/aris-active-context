...
raiz/
  ...
  scripts/                         ← render_boot.py, check_boot_sync.py, assert_mirror_sync.py
...
HISTORICAL_ONLY
SUPERSEDED_BY_LAPIDARIUM_FASE6_GUARDA_TRUE
NOT_CURRENT_STATE

As seções abaixo (a partir de "Estado canônico atual", que documenta o snapshot histórico `IF09_CLOSURE_MILESTONE_MIRROR_SANITY_PACKET`) preservam trilhas e locks anteriores para auditoria. Qualquer menção abaixo deste ponto a `IF09-FIND-001 remains open`, `finding_closed=false`, `remediation_proven=false`, `INF_REVALIDATION_EXECUTION_PACKET` candidate-only ou `revalidation not executed` é histórica e não descreve o estado vivo atual. O estado vivo atual é `LAPIDARIUM_FASE_6_GUARDA_TRUE` conforme `ACTIVE_CONTEXT_STATE.json` e o snapshot no topo deste arquivo.

## Estado canônico atual (histórico — snapshot IF09, superseded)
