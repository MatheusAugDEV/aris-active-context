# ARIS_ROADMAP_EVOLUTION

## Roadmap evolution

- Initial roadmap: 8 phases.
- Sprint Block A: stabilization, tests, imports, lazy loading.
- F14-F21: semantic understanding, evaluation, voice safety, action contracts, context OS.
- Roadmap V4/V6: broader gates, evidence, reliability, packaging, and closure discipline.
- Long-roadmap critique: the gap between current state and final roadmap was large.
- Pre-V6 cockpit/evidence foundation: separated from official V6.
- Official V6: PDF F1-F29.
- F1-F22: covered by reconciliation.
- F23-F28: executed.
- F29: blocked.
- Decision: follow the PDF exactly for official V6 naming.
- V7 is not authorized yet.

## Current roadmap lock

- official V6 naming = F1-F29 only;
- no V6-O-* in official phase fields;
- no V7 before F29 final closure;
- F29 requires Context Control / Obsidian Law;
- F28 technical ready is not V6 closure.
