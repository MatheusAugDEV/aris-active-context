...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_response_evidence_packaging_closure_gate_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Evidence Packaging Closure Gate", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_operator_approval_evidence_route_consolidation_gate", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Evidence Route Consolidation Gate", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_response_evidence_packaging_closure_gate_pass",
        "ready_for_controlled_apply_dry_run_operator_approval_evidence_route_consolidation_gate",
    ...
    _mirror_contains(
        ...
        "Closure scope is route-local only; the broader controlled-apply dry-run track is not closed here.",
    )
    _mirror_contains(
        ROOT / "CONTEXT_INDEX.md",
        "artifact routes are derived from ACTIVE_CONTEXT_STATE.json",
        "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Evidence Packaging Closure Gate",
        "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Evidence Route Consolidation Gate",
        "next-route contract",
    )
    _mirror_contains(
        ...
        json.dumps(
            {
                ...
                "review_result": "evidence packaging closure gate pass",
            },
            indent=2,
        )
    )
