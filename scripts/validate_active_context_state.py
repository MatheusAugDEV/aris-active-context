...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_operator_authorization_packet_planning_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Authorization Packet Planning", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_operator_authorization_packet_readiness_review", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Authorization Packet Readiness Review", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_operator_authorization_packet_planning_pass",
        "ready_for_controlled_apply_dry_run_operator_authorization_packet_readiness_review",
        "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Authorization Packet Readiness Review",
        "Operator Authorization Packet Planning executed as planning-only and did not request operator approval, capture operator approval, grant execution authorization, real dry-run execution, real apply, or approval execution.",
    )
    _mirror_contains(
        ROOT / "NEXT_ACTION.md",
        "Derived mirror from ACTIVE_CONTEXT_STATE.json",
    ...
    _mirror_contains(
        ...
        "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Authorization Packet Planning",
        "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Authorization Packet Readiness Review",
    )
    _mirror_contains(
        ROOT / "ARIS_PHASE_LEDGER.md",
        ...
        json.dumps(
            {
                ...
                "review_result": "operator authorization packet planning pass",
            },
            indent=2,
        )
    )
