...
EXPECTED_STATUS = "aris_infernus_lab_full_scenario_manifest_dataset_review_gate_pass"
EXPECTED_DECISION = "pass"
EXPECTED_LATEST = "ARIS Infernus Lab FULL Scenario Manifest Dataset Review Gate"
EXPECTED_CURRENT_STATUS = "ready_for_aris_infernus_lab_full_fixture_materialization_planning_gate"
EXPECTED_NEXT = "ARIS Infernus Lab FULL Fixture Materialization Planning Gate"
...
def main() -> None:
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Scenario Manifest Dataset Planning Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Scenario Manifest Dataset Planning Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "CURRENT_STATE.md", "ACTIVE_CONTEXT_STATE.json wins", EXPECTED_STATUS, EXPECTED_NEXT, "The 13-scenario manifest and synthetic dataset planning pack passed review", "Synthetic-only boundaries remain explicit", *roadmap_required)
    ...
    _mirror_contains(ROOT / "DECISION_LOCKS.md", EXPECTED_NEXT, "synthetic-only boundaries", "Bedrock remains non-executable and product promotion remains blocked.")
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", EXPECTED_NEXT, "ARIS_INFERNUS_FULL_SCENARIO_MANIFEST_DATASET_REVIEW_GATE.md", "aris_infernus_lab_full_scenario_manifest_dataset_review_gate_coverage_review.json")
    _mirror_contains(ROOT / "ARIS_PHASE_LEDGER.md", EXPECTED_STATUS, EXPECTED_NEXT, "Scenario Manifest Dataset Review Gate Note", *roadmap_required)
    _mirror_contains(ROOT / "README.md", EXPECTED_LATEST, EXPECTED_NEXT, "ARIS_INFERNUS_FULL_SCENARIO_MANIFEST_DATASET_REVIEW_GATE.md")
    ...
    _mirror_contains(ROOT / "ARIS_INFERNUS_FULL_SCENARIO_MANIFEST_DATASET_REVIEW_GATE.md", EXPECTED_LATEST, EXPECTED_NEXT, "The three prior warnings remain resolved", "No fixture materialization.")
    ...
    print(json.dumps({
        ...
        "review_result": "aris infernus lab full scenario manifest dataset review gate pass"
    }, indent=2))
