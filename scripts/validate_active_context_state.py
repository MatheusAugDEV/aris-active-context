...
EXPECTED_STATUS = "aris_infernus_lab_full_fixture_materialization_explicit_operator_authorization_packet_planning_gate_pass"
EXPECTED_DECISION = "pass"
EXPECTED_LATEST = "ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Packet Planning Gate"
EXPECTED_CURRENT_STATUS = "ready_for_aris_infernus_lab_full_fixture_materialization_explicit_operator_authorization_packet_review_gate"
EXPECTED_NEXT = "ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Packet Review Gate"
...
def main() -> None:
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Fixture Materialization Readiness Closure Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Fixture Materialization Readiness Closure Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(
        ...
        EXPECTED_STATUS,
        EXPECTED_NEXT,
        "packet_status_default=not_requested",
        "operator_authorization_requested_now=false",
        "operator_authorization_granted_now=false",
        "operator_identity_placeholder_only=true",
        "operator_identity_captured_now=false",
        "operator_signature_captured_now=false",
    ...
    _mirror_contains(ROOT / "DECISION_LOCKS.md", EXPECTED_NEXT, "packet_status_default=not_requested", "operator_authorization_requested_now=false", "Bedrock remains non-executable and product promotion remains blocked.")
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", EXPECTED_NEXT, "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_EXPLICIT_OPERATOR_AUTHORIZATION_PACKET_PLANNING_GATE.md", "aris_infernus_lab_full_fixture_materialization_explicit_operator_authorization_packet_plan.json")
    _mirror_contains(ROOT / "ARIS_PHASE_LEDGER.md", EXPECTED_STATUS, EXPECTED_NEXT, "Explicit Operator Authorization Packet Planning Gate Note", *roadmap_required)
    _mirror_contains(ROOT / "README.md", EXPECTED_LATEST, EXPECTED_NEXT, "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_EXPLICIT_OPERATOR_AUTHORIZATION_PACKET_PLANNING_GATE.md")
    _mirror_contains(ROOT / "BEDROCK_GATE.md", EXPECTED_LATEST, EXPECTED_NEXT, "Productization remains blocked")
    _mirror_contains(ROOT / "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_EXPLICIT_OPERATOR_AUTHORIZATION_PACKET_PLANNING_GATE.md", "Packet status default: `not_requested`.", "Operator authorization requested now: `False`.", "Operator authorization granted now: `False`.", "Operator identity captured now: `False`.", "Operator signature captured now: `False`.", "Next Recommended Phase", EXPECTED_NEXT)

    stale_route_lines = (
        "- Active next phase: `ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Packet Planning Gate`",
        "Fixture materialization explicit operator authorization packet planning gate next; no Bedrock authorization yet.",
        "Readiness closure gate next; no Bedrock authorization yet.",
    )
    ...
    print(json.dumps({
        ...
        "review_result": "aris infernus lab full fixture materialization explicit operator authorization packet planning gate pass"
    }, indent=2))
