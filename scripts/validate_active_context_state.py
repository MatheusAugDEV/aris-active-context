...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_execution_authorization_planning_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Execution Authorization Planning", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_execution_authorization_planning_readiness_review", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Execution Authorization Planning Readiness Review", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_execution_authorization_planning_pass",
        "ready_for_controlled_apply_dry_run_execution_authorization_planning_readiness_review",
        ...
        json.dumps(
            {
                ...
                "review_result": "execution authorization planning pass",
            },
            indent=2,
        )
    )
