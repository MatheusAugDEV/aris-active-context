...
EXPECTED_STATUS = "aris_infernus_lab_full_fixture_materialization_explicit_operator_authorization_request_review_gate_pass"
EXPECTED_DECISION = "pass"
EXPECTED_LATEST = "ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Request Review Gate"
EXPECTED_CURRENT_STATUS = "ready_for_aris_infernus_lab_full_fixture_materialization_explicit_operator_authorization_grant_planning_gate"
EXPECTED_NEXT = "ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Grant Planning Gate"
...
def main() -> None:
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Request Planning Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Request Planning Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "DECISION_LOCKS.md", EXPECTED_NEXT, "request_reviewed=true", "authorization_requested_now=false", "Bedrock remains non-executable and product promotion remains blocked.")
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", EXPECTED_NEXT, "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_EXPLICIT_OPERATOR_AUTHORIZATION_REQUEST_REVIEW_GATE.md", "aris_infernus_lab_full_fixture_materialization_explicit_operator_authorization_request_review_gate_matrix.json")
    _mirror_contains(ROOT / "ARIS_PHASE_LEDGER.md", EXPECTED_STATUS, EXPECTED_NEXT, "Explicit Operator Authorization Request Review Gate Note", *roadmap_required)
    _mirror_contains(ROOT / "README.md", EXPECTED_LATEST, EXPECTED_NEXT, "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_EXPLICIT_OPERATOR_AUTHORIZATION_REQUEST_REVIEW_GATE.md")
    _mirror_contains(ROOT / "BEDROCK_GATE.md", EXPECTED_LATEST, EXPECTED_NEXT, "Productization remains blocked")
    _mirror_contains(ROOT / "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_EXPLICIT_OPERATOR_AUTHORIZATION_REQUEST_REVIEW_GATE.md", "Request review passed: `True`.", "Request delivered now: `False`.", "Authorization granted now: `False`.", "Next Recommended Phase", EXPECTED_NEXT)

    stale_route_lines = (
        "- Active next phase: `ARIS Infernus Lab FULL Fixture Materialization Explicit Operator Authorization Request Review Gate`",
        "Fixture materialization explicit operator authorization request review gate next; no Bedrock authorization yet.",
    )
    ...
    print(json.dumps({
        ...
        "review_result": "aris infernus lab full fixture materialization explicit operator authorization request review gate pass"
    }, indent=2))
