...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_response_simulation_readiness_review_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Simulation Readiness Review", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_operator_approval_response_simulation_final_review_gate", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Simulation Final Review Gate", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_response_simulation_readiness_review_pass",
        "ready_for_controlled_apply_dry_run_operator_approval_response_simulation_final_review_gate",
        ...
        json.dumps(
            {
                ...
                "review_result": "operator approval response simulation readiness review pass",
            },
            indent=2,
        )
    )
