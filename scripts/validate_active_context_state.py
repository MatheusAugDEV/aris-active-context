...
def main() -> None:
    ...
    _require(state["status"] == "lab_simulation_closure_post_review_routing_decision_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Simulation Closure Post-Review Routing Decision", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_aris_infernus_lab_full_opening_gate", "unexpected current status")
    _require(state["active_next_phase"] == "ARIS Infernus Lab FULL Opening Gate", "unexpected next route")
    ...
    _require(state["next_action"]["planning_only"] is True, "next route must remain planning_only")
    _require(state["next_action"]["review_only"] is False, "next route must not remain review_only")
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Real Execution Lab Simulation Closure Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Real Execution Lab Simulation Closure Gate", "unexpected last transition from phase")
        ...
        json.dumps(
            {
                ...
                "review_result": "lab simulation closure post-review routing decision pass",
            },
            indent=2,
        )
    )
