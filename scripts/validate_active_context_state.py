...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_response_evidence_packaging_planning_correction_readiness_review_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Evidence Packaging Planning Correction Readiness Review", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_operator_approval_response_evidence_packaging_final_review_gate", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Evidence Packaging Final Review Gate", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_response_evidence_packaging_planning_correction_readiness_review_pass",
        "ready_for_controlled_apply_dry_run_operator_approval_response_evidence_packaging_final_review_gate",
        ...
        json.dumps(
            {
                ...
                "review_result": "planning correction readiness review pass",
            },
            indent=2,
        )
    )
