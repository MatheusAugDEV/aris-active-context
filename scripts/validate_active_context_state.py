...
EXPECTED_STATUS = "aris_infernus_lab_full_controlled_fixture_materialization_apply_planning_gate_pass"
EXPECTED_DECISION = "pass"
EXPECTED_LATEST = "ARIS Infernus Lab FULL Controlled Fixture Materialization Apply Planning Gate"
EXPECTED_CURRENT_STATUS = "ready_for_aris_infernus_lab_full_controlled_fixture_materialization_apply_review_gate"
EXPECTED_NEXT = "ARIS Infernus Lab FULL Controlled Fixture Materialization Apply Review Gate"
...
def main() -> None:
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Controlled Fixture Materialization Authorization Review Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Controlled Fixture Materialization Authorization Review Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "DECISION_LOCKS.md", EXPECTED_NEXT, "Apply planning keeps `real_apply_authorized=false`, `apply_execution_allowed=false`, `fixture_materialization_allowed=false`, and future human approval requirements non-bypassable.", "Bedrock remains non-executable and product promotion remains blocked.")
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", EXPECTED_NEXT, "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_APPLY_PLANNING_GATE.md", "aris_infernus_lab_full_controlled_fixture_materialization_apply_plan.json")
    _mirror_contains(ROOT / "ARIS_PHASE_LEDGER.md", EXPECTED_STATUS, EXPECTED_NEXT, "Controlled Fixture Materialization Apply Planning Gate Note", *roadmap_required)
    _mirror_contains(ROOT / "README.md", EXPECTED_LATEST, EXPECTED_NEXT, "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_APPLY_PLANNING_GATE.md")
    ...
    _mirror_contains(ROOT / "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_APPLY_PLANNING_GATE.md", EXPECTED_LATEST, EXPECTED_NEXT, "Real apply authorized: `False`.", "Apply execution allowed: `False`.", "Human approval collected now: `False`.", "Dry-run executed now: `False`.")
    ...
    print(json.dumps({
        ...
        "review_result": "aris infernus lab full controlled fixture materialization apply planning gate pass"
    }, indent=2))
