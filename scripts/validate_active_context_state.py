...
EXPECTED_STATUS = "aris_infernus_lab_full_controlled_fixture_materialization_authorization_review_gate_pass"
EXPECTED_DECISION = "pass"
EXPECTED_LATEST = "ARIS Infernus Lab FULL Controlled Fixture Materialization Authorization Review Gate"
EXPECTED_CURRENT_STATUS = "ready_for_aris_infernus_lab_full_controlled_fixture_materialization_apply_planning_gate"
EXPECTED_NEXT = "ARIS Infernus Lab FULL Controlled Fixture Materialization Apply Planning Gate"
...
def main() -> None:
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Controlled Fixture Materialization Authorization Planning Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Controlled Fixture Materialization Authorization Planning Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "DECISION_LOCKS.md", EXPECTED_NEXT, "Authorization review keeps `authorization_granted=false`, `fixture_materialization_allowed=false`, and future human approval requirements non-bypassable.", "Bedrock remains non-executable and product promotion remains blocked.")
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", EXPECTED_NEXT, "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_AUTHORIZATION_REVIEW_GATE.md", "aris_infernus_lab_full_controlled_fixture_materialization_authorization_review_gate_contract_review.json")
    _mirror_contains(ROOT / "ARIS_PHASE_LEDGER.md", EXPECTED_STATUS, EXPECTED_NEXT, "Controlled Fixture Materialization Authorization Review Gate Note", *roadmap_required)
    _mirror_contains(ROOT / "README.md", EXPECTED_LATEST, EXPECTED_NEXT, "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_AUTHORIZATION_REVIEW_GATE.md")
    ...
    _mirror_contains(ROOT / "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_AUTHORIZATION_REVIEW_GATE.md", EXPECTED_LATEST, EXPECTED_NEXT, "Authorization granted: `False`.", "Human approval collected now: `False`.")
    ...
    print(json.dumps({
        ...
        "review_result": "aris infernus lab full controlled fixture materialization authorization review gate pass"
    }, indent=2))
