...
def main() -> None:
    ...
    _require(state["status"] == "aris_infernus_lab_full_contract_schema_hardening_gate_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "ARIS Infernus Lab FULL Contract Schema Hardening Gate", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_aris_infernus_lab_full_contract_schema_hardening_review", "unexpected current status")
    _require(state["active_next_phase"] == "ARIS Infernus Lab FULL Contract Schema Hardening Review", "unexpected next route")
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Contract Foundation Review", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Contract Foundation Review", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "CURRENT_STATE.md", "ARIS Infernus Lab FULL Contract Schema Hardening Gate", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "Six-phase route closed: `True`")
    _mirror_contains(ROOT / "NEXT_ACTION.md", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "Review-only: `true`", "Execution authorization: `false`")
    ...
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", "ARIS Infernus Lab FULL Contract Schema Hardening Gate", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "Schema hardening inventory")
    _mirror_contains(ROOT / "README.md", "ARIS Infernus Lab FULL Contract Schema Hardening Gate", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "Residual warnings remain carried forward explicitly.")
    _mirror_contains(ROOT / "ROADMAP_CANONICAL.md", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "Contract Schema Hardening Review remains review-only.")
    _mirror_contains(ROOT / "LAB_VERDICTS.md", "ARIS Infernus Lab FULL Contract Schema Hardening Gate - Bedrock Preparation Exception Record", "ARIS Infernus Lab FULL Contract Schema Hardening Review")
    ...
    print(json.dumps({"decision": "pass", "validated_paths": [str(ROOT / "CURRENT_STATE.md"), str(ROOT / "NEXT_ACTION.md"), str(ROOT / "DECISION_LOCKS.md"), str(ROOT / "CONTEXT_INDEX.md"), str(ROOT / "ARIS_PHASE_LEDGER.md"), str(ROOT / "ROADMAP_CANONICAL.md"), str(ROOT / "README.md"), str(ROOT / "LAB_VERDICTS.md")], "review_result": "aris infernus lab full contract schema hardening gate pass"}, indent=2))
