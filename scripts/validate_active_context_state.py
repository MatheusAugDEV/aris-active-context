...
def main() -> None:
    ...
    _require(state["status"] == "aris_infernus_lab_full_opening_gate_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "ARIS Infernus Lab FULL Opening Gate", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_aris_infernus_lab_full_readiness_review", "unexpected current status")
    _require(state["active_next_phase"] == "ARIS Infernus Lab FULL Readiness Review", "unexpected next route")
    _require(state["active_next_phase_class"] == "readiness_gate", "unexpected next route class")
    ...
    _require(state["next_action"]["planning_only"] is False, "next route must not remain planning_only")
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "Lab Simulation Closure Post-Review Routing Decision", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "Lab Simulation Closure Post-Review Routing Decision", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "CURRENT_STATE.md", "ARIS Infernus Lab FULL Opening Gate", "ARIS Infernus Lab FULL Readiness Review", "Six-phase route closed: `True`")
    _mirror_contains(ROOT / "NEXT_ACTION.md", "ARIS Infernus Lab FULL Readiness Review", "Review-only: `true`", "Execution authorization: `false`")
    ...
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", "ARIS Infernus Lab FULL Opening Gate", "ARIS Infernus Lab FULL Readiness Review", "Official bot taxonomy contract")
    _mirror_contains(ROOT / "README.md", "ARIS Infernus Lab FULL Opening Gate", "ARIS Infernus Lab FULL Readiness Review", "Residual warnings remain carried forward explicitly.")
    _mirror_contains(ROOT / "ROADMAP_CANONICAL.md", "ARIS Infernus Lab FULL Readiness Review", "Lab Simulation route is closed and does not create phase 7.")
    _mirror_contains(ROOT / "LAB_VERDICTS.md", "ARIS Infernus Lab FULL Opening Gate - Bedrock Preparation Exception Record", "ARIS Infernus Lab FULL Readiness Review")
    ...
    print(json.dumps({"decision": "pass", "validated_paths": [str(ROOT / "CURRENT_STATE.md"), str(ROOT / "NEXT_ACTION.md"), str(ROOT / "DECISION_LOCKS.md"), str(ROOT / "CONTEXT_INDEX.md"), str(ROOT / "ARIS_PHASE_LEDGER.md"), str(ROOT / "ROADMAP_CANONICAL.md"), str(ROOT / "README.md"), str(ROOT / "LAB_VERDICTS.md")], "review_result": "aris infernus lab full opening gate pass"}, indent=2))
