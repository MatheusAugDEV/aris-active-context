...
EXPECTED_STATUS = "aris_infernus_lab_full_fixture_materialization_final_authorization_packet_planning_gate_pass"
EXPECTED_DECISION = "pass"
EXPECTED_LATEST = "ARIS Infernus Lab FULL Fixture Materialization Final Authorization Packet Planning Gate"
EXPECTED_CURRENT_STATUS = "ready_for_aris_infernus_lab_full_fixture_materialization_final_authorization_packet_review_gate"
EXPECTED_NEXT = "ARIS Infernus Lab FULL Fixture Materialization Final Authorization Packet Review Gate"
...
def main() -> None:
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Controlled Fixture Materialization Apply Review Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Controlled Fixture Materialization Apply Review Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "DECISION_LOCKS.md", EXPECTED_NEXT, "Final authorization packet planning keeps `authorization_packet_actionable=false`, `authorization_granted=false`, `human_approval_collected_now=false`, `real_apply_authorized=false`, `apply_execution_allowed=false`, and `fixture_materialization_allowed=false`.", "Bedrock remains non-executable and product promotion remains blocked.")
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", EXPECTED_NEXT, "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_FINAL_AUTHORIZATION_PACKET_PLANNING_GATE.md", "aris_infernus_lab_full_fixture_materialization_final_authorization_packet_plan.json")
    _mirror_contains(ROOT / "ARIS_PHASE_LEDGER.md", EXPECTED_STATUS, EXPECTED_NEXT, "Fixture Materialization Final Authorization Packet Planning Gate Note", *roadmap_required)
    _mirror_contains(ROOT / "README.md", EXPECTED_LATEST, EXPECTED_NEXT, "ARIS_INFERNUS_FULL_FIXTURE_MATERIALIZATION_FINAL_AUTHORIZATION_PACKET_PLANNING_GATE.md")
    ...
    print(json.dumps({
        ...
        "review_result": "aris infernus lab full fixture materialization final authorization packet planning gate pass"
    }, indent=2))
