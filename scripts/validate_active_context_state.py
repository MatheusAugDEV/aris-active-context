...
def main() -> None:
    ...
    _require(state["status"] == "aris_infernus_lab_full_contract_schema_hardening_review_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "ARIS Infernus Lab FULL Contract Schema Hardening Review", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_aris_infernus_lab_full_contract_schema_enforcement_planning_gate", "unexpected current status")
    _require(state["active_next_phase"] == "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate", "unexpected next route")
    ...
    _require(state["history_summary"]["previous_execution_phase"] == "ARIS Infernus Lab FULL Contract Schema Hardening Gate", "unexpected previous execution phase")
    _require(state["last_transition"]["from_phase"] == "ARIS Infernus Lab FULL Contract Schema Hardening Gate", "unexpected last transition from phase")
    ...
    _mirror_contains(ROOT / "CURRENT_STATE.md", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate", "Six-phase route closed: `True`")
    _mirror_contains(ROOT / "NEXT_ACTION.md", "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate", "Planning-only: `true`", "Execution authorization: `false`")
    ...
    _mirror_contains(ROOT / "CONTEXT_INDEX.md", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate", "Artifact inventory")
    _mirror_contains(ROOT / "README.md", "ARIS Infernus Lab FULL Contract Schema Hardening Review", "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate", "Residual warnings remain carried forward explicitly.")
    _mirror_contains(ROOT / "ROADMAP_CANONICAL.md", "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate", "Contract Schema Enforcement Planning Gate remains planning-only.")
    _mirror_contains(ROOT / "LAB_VERDICTS.md", "ARIS Infernus Lab FULL Contract Schema Hardening Review - Bedrock Preparation Exception Record", "ARIS Infernus Lab FULL Contract Schema Enforcement Planning Gate")
    ...
    print(json.dumps({"decision": "pass", "validated_paths": [str(ROOT / "CURRENT_STATE.md"), str(ROOT / "NEXT_ACTION.md"), str(ROOT / "DECISION_LOCKS.md"), str(ROOT / "CONTEXT_INDEX.md"), str(ROOT / "ARIS_PHASE_LEDGER.md"), str(ROOT / "ROADMAP_CANONICAL.md"), str(ROOT / "README.md"), str(ROOT / "LAB_VERDICTS.md")], "review_result": "aris infernus lab full contract schema hardening review pass"}, indent=2))
