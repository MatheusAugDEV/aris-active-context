...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_authorization_prerequisites_final_review_gate_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Authorization Prerequisites Final Review Gate", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_real_execution_request_planning_gate", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Real Execution Request Planning Gate", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_authorization_prerequisites_final_review_gate_pass",
        "ready_for_controlled_apply_dry_run_real_execution_request_planning_gate",
    ...
    _mirror_contains(
        ...
        "Lab Real Simulation Pack Controlled Apply Dry-Run Real Execution Request Planning Gate",
        "Authorization prerequisites final review next-route contract",
    )
    _mirror_contains(
        ROOT / "ARIS_PHASE_LEDGER.md",
        ...
        json.dumps(
            {
                ...
                "review_result": "authorization prerequisites final review gate pass",
            },
            indent=2,
        )
    )
