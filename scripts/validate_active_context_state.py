...
def main() -> None:
    ...
    _require(state["status"] == "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_request_simulation_final_review_gate_pass", "unexpected status")
    _require(state["decision"] == "pass", "unexpected decision")
    _require(state["latest_completed_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Request Simulation Final Review Gate", "unexpected latest completed phase")
    _require(state["current_status"] == "ready_for_controlled_apply_dry_run_operator_approval_response_simulation_planning", "unexpected current status")
    _require(state["active_next_phase"] == "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Simulation Planning", "unexpected next phase")
    ...
    _mirror_contains(
        ...
        "lab_real_simulation_pack_controlled_apply_dry_run_operator_approval_request_simulation_final_review_gate_pass",
        "ready_for_controlled_apply_dry_run_operator_approval_response_simulation_planning",
        "Lab Real Simulation Pack Controlled Apply Dry-Run Operator Approval Response Simulation Planning",
        "Operator Approval Request Simulation Final Review Gate executed as review-only and revalidated the planning and readiness-review artifacts as one coherent synthetic-only, not-sent, non-contacting, non-capturing, and non-authorizing evidence chain.",
    )
    _mirror_contains(
        ROOT / "NEXT_ACTION.md",
        "Derived mirror from ACTIVE_CONTEXT_STATE.json",
        ...
        json.dumps(
            {
                ...
                "review_result": "operator approval request simulation final review gate pass",
            },
            indent=2,
        )
    )
