...
class TestCanonicalState(unittest.TestCase):
    def test_canonical_state_passes(self) -> None:
        ...
        self.assertEqual(data["latest_completed_phase"], "ARIS Infernus Lab FULL Controlled Fixture Materialization Authorization Review Gate")
        self.assertEqual(data["active_next_phase"], "ARIS Infernus Lab FULL Controlled Fixture Materialization Apply Planning Gate")
...
class TestMirrorGuards(unittest.TestCase):
    ...
    def test_authorization_review_artifact_without_future_human_approval_phrase_blocks(self) -> None:
        state = _load_state()
        original = (ROOT / "ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_AUTHORIZATION_REVIEW_GATE.md").read_text(encoding="utf-8")
        corrupted = original.replace(
        ...
        rc, _, stderr = _run_validator_with_state(
            state,
            extra_files={"ARIS_INFERNUS_FULL_CONTROLLED_FIXTURE_MATERIALIZATION_AUTHORIZATION_REVIEW_GATE.md": corrupted},
        )
        self.assertNotEqual(rc, 0, "authorization review artifact without future-only human approval phrase must be blocked.\n" + stderr)
