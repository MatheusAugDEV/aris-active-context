...
def test_validator_reports_canonical_inf_revalidation_summary():
    r = subprocess.run(
        ["python3", "scripts/validate_active_context_state.py"],
        capture_output=True,
        text=True,
    ...
    assert summary["phase_id"] == "INF_REVALIDATION_ROUTE_ADMISSION_PACKET"
...
def test_state_separates_historical_and_planned_scenario_counts():
    ...
    assert state["current_phase_id"] == "INF_REVALIDATION_ROUTE_ADMISSION_PACKET"
    ...
    assert state["current_status"] == "inf_revalidation_route_admission_opened"
    assert state["latest_completed_phase"] == "INF Revalidation Route Admission Packet"
    assert state["latest_completed_status"] == "inf_revalidation_route_admission_opened"
