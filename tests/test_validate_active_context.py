...
def test_validator_reports_canonical_purg04_summary():
    ...
    assert summary["phase_id"] == "PURG_RESIDUAL_RISK_CARRY_FORWARD_PACKET"
...
def test_state_separates_historical_and_planned_scenario_counts():
    ...
    assert state["current_phase_id"] == "PURG_RESIDUAL_RISK_CARRY_FORWARD_PACKET"
    ...
    assert state["current_status"] == "purg_residual_risk_carry_forward_route_opening_pass"
    assert state["latest_completed_phase"] == "PURG Residual Risk Carry-Forward Packet"
    assert state["latest_completed_status"] == "purg_residual_risk_carry_forward_route_opening_pass"
