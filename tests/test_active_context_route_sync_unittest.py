...
class ActiveContextRouteSyncTests(unittest.TestCase):
    ...
    def test_current_route_tracks_inf_revalidation_readiness_packet_sync(self):
        state = json.loads((ROOT / "ACTIVE_CONTEXT_STATE.json").read_text(encoding="utf-8"))
        self.assertEqual(state["phase_id"], "INF_REVALIDATION_READINESS_PACKET")
        self.assertEqual(state["current_phase_id"], "INF_REVALIDATION_READINESS_PACKET")
        self.assertEqual(state["previous_phase_id"], "INF_REVALIDATION_ROUTE_ADMISSION_PACKET")
        self.assertEqual(state["phase_class"], "infernus_revalidation_readiness")
        self.assertEqual(state["latest_completed_phase"], "INF Revalidation Readiness Packet")
        self.assertEqual(state["latest_completed_status"], "inf_revalidation_readiness_opened")
        ...
        self.assertEqual(state["status"], "inf_revalidation_readiness_opened")
        self.assertEqual(state["current_live_route"]["decision"], "pass")
        self.assertEqual(state["current_live_route"]["status"], "inf_revalidation_readiness_opened")
    ...
    def test_transition_table_has_no_successor_for_inf_revalidation_readiness_packet(self):
        module = self._load_validator_module()
        self.assertIsNone(module._get_transition_row("INF_REVALIDATION_READINESS_PACKET", "pass"))
