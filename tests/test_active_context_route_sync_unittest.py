...
class ActiveContextRouteSyncTests(unittest.TestCase):
    ...
    def test_current_route_tracks_if08_w6_final_audit_controlled_execution_sync(self):
        state = json.loads(Path("ACTIVE_CONTEXT_STATE.json").read_text(encoding="utf-8"))
        self.assertEqual(state["latest_completed_phase"], "IF-08 W6 Final Audit Controlled Execution")
        self.assertEqual(state["latest_completed_status"], "if08_w6_final_audit_controlled_execution_pass")
        self.assertEqual(state["latest_completed_project_commit_sha"], "eae468c79687474de086c984b55a3f7ff47d73f7")
        self.assertEqual(
        ...
        self.assertTrue(state["latest_completed_no_execution"]["preflight_readiness_verified"])
        self.assertEqual(state["latest_completed_no_execution"]["execution_scope"], "synthetic_isolated_lab_only")
        self.assertEqual(state["latest_completed_no_execution"]["source_preflight_status"], "if08_w6_final_audit_preflight_readiness_pass")
        self.assertEqual(state["latest_completed_no_execution"]["source_project_sha_verified_by_packet"], "0358de95fd78c41fad2e257fec399d85e74193ce")
        self.assertEqual(state["latest_completed_no_execution"]["source_active_context_sha_verified_by_packet"], "36f4891c33d6a81eae74df1cfa5d3717cd6b4bc5")
        self.assertEqual(state["latest_completed_no_execution"]["source_project_sha_recorded_in_active_context"], "0358de95c7fb80d06871a20ae46b8fbc3174c5d7")
        self.assertTrue(state["latest_completed_no_execution"]["source_project_sha_drift_recorded"])
        self.assertTrue(state["latest_completed_no_execution"]["source_project_sha_drift_resolved_via_primary_refs"])
        self.assertEqual(state["latest_completed_no_execution"]["previous_phase_verified"], "IF-08 W6 Final Audit Preflight Readiness")
        self.assertEqual(state["latest_completed_no_execution"]["previous_status_verified"], "if08_w6_final_audit_preflight_readiness_pass")
        self.assertEqual(state["latest_completed_no_execution"]["sirene_oracle_mode"], "synthetic_transcript_only")
        self.assertEqual(state["latest_completed_no_execution"]["w6_readiness_state"], "completed_synthetic_controlled_execution")
        self.assertFalse(state["latest_completed_no_execution"]["w6_preparation_allowed_next"])
        ...
        self.assertEqual(state["latest_completed_no_execution"]["ttr_required"], 0)
        self.assertEqual(state["latest_completed_no_execution"]["ttr_observed"], 0)
        self.assertEqual(state["latest_completed_no_execution"]["har_required"], 1.0)
        self.assertEqual(state["latest_completed_no_execution"]["har_observed"], 1.0)
        ...
        self.assertTrue(state["latest_completed_no_execution"]["anti_theater_review_passed"])
