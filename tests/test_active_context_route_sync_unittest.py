...
class ActiveContextRouteSyncTests(unittest.TestCase):
    ...
    def test_current_route_tracks_inf_revalidation_execution_packet_sync(self):
        ...
        self.assertEqual(state["previous_phase_id"], "LAPIDARIUM")
        self.assertEqual(state["phase_class"], "lapidarium_remediation")
        self.assertEqual(state["status"], "lapidarium_fase_3_cache_verificacao_pass")
        self.assertEqual(state["next_phase"], "LAPIDARIUM_FASE_4_REVISAO_CODIGO_GENUINO")
        self.assertEqual(state["active_next_phase"], "LAPIDARIUM_FASE_4_REVISAO_CODIGO_GENUINO")
        ...
        self.assertEqual(state["latest_completed_phase"], "Lapidarium Fase 3 - Verificação de Cache")
        self.assertEqual(state["latest_completed_status"], "lapidarium_fase_3_cache_verificacao_pass")
        self.assertEqual(state["current_live_route"]["active_next_phase"], "LAPIDARIUM_FASE_4_REVISAO_CODIGO_GENUINO")
        self.assertEqual(state["current_live_route"]["active_next_phase_class"], "lapidarium_remediation")
        self.assertEqual(state["current_live_route"]["current_status"], "lapidarium_fase_3_cache_verificacao_pass")
        self.assertEqual(state["current_live_route"]["status"], "lapidarium_fase_3_cache_verificacao_pass")
        self.assertEqual(state["current_live_route"]["latest_completed_phase"], "Lapidarium Fase 3 - Verificação de Cache")
        self.assertEqual(state["current_live_route"]["latest_completed_status"], "lapidarium_fase_3_cache_verificacao_pass")
        self.assertEqual(state["next_action"]["phase"], "LAPIDARIUM_FASE_4_REVISAO_CODIGO_GENUINO")
        self.assertEqual(state["next_action"]["phase_class"], "lapidarium_remediation")
        self.assertEqual(state["next_action"]["status"], "lapidarium_fase_3_cache_verificacao_pass")
        self.assertFalse(state["next_action"]["planning_only"])
        self.assertFalse(state["next_action"]["review_only"])
        self.assertIn("Phase 1.5 remains closed and phase 3 is now the latest completed step.", state["next_action"]["notes"])
